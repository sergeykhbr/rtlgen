...
TEXT();
    ...
    CASE (cfg->CSR_misa);
        ...
        TEXT("     24 Y Reserved");
        TEXT("     25 Z Reserve");
        SETONE(BIT(comb.vb_rdata, 'A' - 'A'));
        SETONE(BIT(comb.vb_rdata, 'I' - 'A'));
        SETONE(BIT(comb.vb_rdata, 'M' - 'A'));
        SETONE(BIT(comb.vb_rdata, 'U' - 'A'));
        SETONE(BIT(comb.vb_rdata, 'C' - 'A'));
        IF (cfg->CFG_HW_FPU_ENABLE);
            SETONE(BIT(comb.vb_rdata, 'D' - 'A'));
        ENDIF();
        ENDCASE();
