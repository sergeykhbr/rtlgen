...
afifo_xslv::afifo_xslv(GenObject *parent, const char *name, const char *comment) :
    ModuleObject(parent, "afifo_xslv", name, comment),
    // Generic parameters
    ...
    // Ports
    i_xslv_nrst(this, "i_xslv_nrst", "1", "Input requests reset (from master to this slave)"),
    i_xslv_clk(this, "i_xslv_clk", "1", "Input requests clock (from master to this slave)"),
    i_xslvi(this, "i_xslvi", "Input slave interface"),
    o_xslvo(this, "o_xslvo", "Response on input slave reuqest"),
    i_xmst_nrst(this, "i_xmst_nrst", "1", "Output request reset"),
    i_xmst_clk(this, "i_xmst_clk", "1", "Output request clock"),
    o_xmsto(this, "o_xmsto", "Output request to connected slave"),
    i_xmsti(this, "i_xmsti", "Response from the connected slave"),
    // params
    AR_REQ_WIDTH(this, "AR_REQ_WIDTH", &CALCWIDTHx(11, &i_xslvi.ar_bits.addr,
                                                       &i_xslvi.ar_bits.len,
                                                       ...
                                                       &i_xslvi.ar_id,
                                                       &i_xslvi.ar_user)),
    AW_REQ_WIDTH(this, "AW_REQ_WIDTH", &CALCWIDTHx(11, &i_xslvi.aw_bits.addr,
                                                       &i_xslvi.aw_bits.len,
                                                       ...
                                                       &i_xslvi.aw_id,
                                                       &i_xslvi.aw_user)),
    W_REQ_WIDTH(this, "W_REQ_WIDTH", &CALCWIDTHx(4, &i_xslvi.w_data,
                                                    &i_xslvi.w_last,
                                                    &i_xslvi.w_strb,
                                                    &i_xslvi.w_user)),
    R_RESP_WIDTH(this, "R_RESP_WIDTH", &CALCWIDTHx(5, &i_xmsti.r_resp,
                                                      &i_xmsti.r_data,
                                                      &i_xmsti.r_last,
                                                      &i_xmsti.r_id,
                                                      &i_xmsti.r_user)),
    B_RESP_WIDTH(this, "B_RESP_WIDTH", &CALCWIDTHx(3, &i_xmsti.b_resp,
                                                      &i_xmsti.b_id,
                                                      &i_xmsti.b_user)),
    // Singals:
    // registers
...
void afifo_xslv::proc_comb() {
    TEXT("ar channel write side:");
    SETVAL(w_req_ar_valid_i, i_xslvi.ar_valid);
    SETVAL(comb.vb_xslvo.ar_ready, w_req_ar_wready_o);
    SETVAL(wb_req_ar_payload_i, CCx(11, &i_xslvi.ar_bits.addr,
                                        &i_xslvi.ar_bits.len,
                                        &i_xslvi.ar_bits.size,
                                        &i_xslvi.ar_bits.burst,
                                        &i_xslvi.ar_bits.lock,
    ...
    SETVAL(comb.vb_xmsto.ar_valid, w_req_ar_rvalid_o);
    SETVAL(w_req_ar_rd_i, i_xmsti.ar_ready);
    SPLx(wb_req_ar_payload_o, 11, &comb.vb_xmsto.ar_bits.addr,
                                  &comb.vb_xmsto.ar_bits.len,
                                  &comb.vb_xmsto.ar_bits.size,
                                  &comb.vb_xmsto.ar_bits.burst,
                                  &comb.vb_xmsto.ar_bits.lock,
                                  &comb.vb_xmsto.ar_bits.cache,
                                  &comb.vb_xmsto.ar_bits.prot,
                                  &comb.vb_xmsto.ar_bits.qos,
                                  &comb.vb_xmsto.ar_bits.region,
                                  &comb.vb_xmsto.ar_id,
                                  &comb.vb_xmsto.ar_user);


    TEXT();
    TEXT("aw channel write side:");
    SETVAL(w_req_aw_valid_i, i_xslvi.aw_valid);
    SETVAL(comb.vb_xslvo.aw_ready, w_req_aw_wready_o);
    SETVAL(wb_req_aw_payload_i, CCx(11, &i_xslvi.aw_bits.addr,
                                        &i_xslvi.aw_bits.len,
                                        &i_xslvi.aw_bits.size,
                                        &i_xslvi.aw_bits.burst,
                                        &i_xslvi.aw_bits.lock,
    ...
    SETVAL(comb.vb_xmsto.aw_valid, w_req_aw_rvalid_o);
    SETVAL(w_req_aw_rd_i, i_xmsti.aw_ready);
    SPLx(wb_req_aw_payload_o, 11, &comb.vb_xmsto.aw_bits.addr,
                                  &comb.vb_xmsto.aw_bits.len,
                                  &comb.vb_xmsto.aw_bits.size,
                                  &comb.vb_xmsto.aw_bits.burst,
                                  &comb.vb_xmsto.aw_bits.lock,
                                  &comb.vb_xmsto.aw_bits.cache,
                                  &comb.vb_xmsto.aw_bits.prot,
                                  &comb.vb_xmsto.aw_bits.qos,
                                  &comb.vb_xmsto.aw_bits.region,
                                  &comb.vb_xmsto.aw_id,
                                  &comb.vb_xmsto.aw_user);

    TEXT();
    TEXT("w channel write side:");
    SETVAL(w_req_w_valid_i, i_xslvi.w_valid);
    SETVAL(comb.vb_xslvo.w_ready, w_req_w_wready_o);
    SETVAL(wb_req_w_payload_i, CCx(4, &i_xslvi.w_data,
                                      &i_xslvi.w_last,
                                      &i_xslvi.w_strb,
                                      &i_xslvi.w_user));
    TEXT("w channel read side:");
    SETVAL(comb.vb_xmsto.w_valid, w_req_w_rvalid_o);
    SETVAL(w_req_w_rd_i, i_xmsti.w_ready);
    SPLx(wb_req_w_payload_o, 4, &comb.vb_xmsto.w_data,
                                 &comb.vb_xmsto.w_last,
                                 &comb.vb_xmsto.w_strb,
                                 &comb.vb_xmsto.w_user);

    TEXT();
    TEXT("r channel write side:");
    SETVAL(w_resp_r_valid_i, i_xmsti.r_valid);
    SETVAL(comb.vb_xmsto.r_ready, w_resp_r_wready_o);
    SETVAL(wb_resp_r_payload_i, CCx(5, &i_xmsti.r_resp,
                                       &i_xmsti.r_data,
                                       &i_xmsti.r_last,
                                       &i_xmsti.r_id,
                                       &i_xmsti.r_user));
    TEXT("r channel read side:");
    SETVAL(comb.vb_xslvo.r_valid, w_resp_r_rvalid_o);
    SETVAL(w_resp_r_rd_i, i_xslvi.r_ready);
    SPLx(wb_resp_r_payload_o, 5, &comb.vb_xslvo.r_resp,
                                 &comb.vb_xslvo.r_data,
                                 &comb.vb_xslvo.r_last,
                                 &comb.vb_xslvo.r_id,
                                 &comb.vb_xslvo.r_user);

    TEXT();
    TEXT("b channel write side:");
    SETVAL(w_resp_b_valid_i, i_xmsti.b_valid);
    SETVAL(comb.vb_xmsto.b_ready, w_resp_b_wready_o);
    SETVAL(wb_resp_b_payload_i, CCx(3, &i_xmsti.b_resp,
                                       &i_xmsti.b_id,
                                       &i_xmsti.b_user));
    TEXT("b channel read side:");
    SETVAL(comb.vb_xslvo.b_valid, w_resp_b_rvalid_o);
    SETVAL(w_resp_b_rd_i, i_xslvi.b_ready);
    SPLx(wb_resp_b_payload_o, 3, &comb.vb_xslvo.b_resp,
                                 &comb.vb_xslvo.b_id,
                                 &comb.vb_xslvo.b_user);

    TEXT();
    SETVAL(o_xslvo, comb.vb_xslvo);
