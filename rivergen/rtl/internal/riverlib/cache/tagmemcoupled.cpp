    ...
    memx(this, "memx", "MemTotal", NO_COMMENT)
{
    ...
    GenObject &i = FORGEN ("i", CONST("0"), CONST("MemTotal"), "++", new StringConst("memgen"));
        NEW(memx, memx.getName().c_str(), &i);
            ...
            CONNECT(memx, &i, memx.i_direct_access, ARRITEM_B(linei, i, linei.direct_access));
            CONNECT(memx, &i, memx.i_invalidate, ARRITEM_B(linei, i, linei.invalidate));
            CONNECT(memx, &i, memx.i_re, ARRITEM_B(linei, i, linei.re));
            CONNECT(memx, &i, memx.i_we, ARRITEM_B(linei, i, linei.we));
            CONNECT(memx, &i, memx.i_addr, ARRITEM_B(linei, i, linei.addr));
            CONNECT(memx, &i, memx.i_wdata, ARRITEM_B(linei, i, linei.wdata));
            CONNECT(memx, &i, memx.i_wstrb, ARRITEM_B(linei, i, linei.wstrb));
            CONNECT(memx, &i, memx.i_wflags, ARRITEM_B(linei, i, linei.wflags));
            CONNECT(memx, &i, memx.o_raddr, ARRITEM_B(lineo, i, lineo.raddr));
            CONNECT(memx, &i, memx.o_rdata, ARRITEM_B(lineo, i, lineo.rdata));
            CONNECT(memx, &i, memx.o_rflags, ARRITEM_B(lineo, i, lineo.rflags));
            CONNECT(memx, &i, memx.o_hit, ARRITEM_B(lineo, i, lineo.hit));
            CONNECT(memx, &i, memx.i_snoop_addr, ARRITEM_B(linei, i, linei.snoop_addr));
            CONNECT(memx, &i, memx.o_snoop_ready, ARRITEM_B(lineo, i, lineo.snoop_ready));
            CONNECT(memx, &i, memx.o_snoop_flags, ARRITEM_B(lineo, i, lineo.snoop_flags));
        ENDNEW();
